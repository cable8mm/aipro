...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('order_shipments', function (Blueprint $table) {
            ...
            $table->integer('optionPrice')->nullable()->comment('주문선택사항금액');
            $table->string('additionalOption', 255)->nullable()->comment('추가구매옵션');
            $table->integer('additionalOptionPrice')->nullable()->comment('추가구매옵션금액');
            $table->integer('costPrice')->nullable()->comment('원가');
            $table->integer('fixedPrice')->nullable()->comment('공급가');
            $table->integer('totalPrice')->nullable()->comment('판매가');
            $table->integer('amount')->nullable()->comment('주문수량');
            $table->integer('totalAmount')->nullable()->comment('총주문수량');
            $table->integer('confirmAmount')->nullable();
            $table->string('deliveryType', 255)->nullable()->comment('배송방법(원본)');
            $table->integer('deliveryPrice')->nullable()->comment('배송비금액');
            $table->integer('totalDeliveryPrice')->nullable()->comment('총배송비금액(묶음후)');
            ...
            $table->integer('inventory')->nullable()->comment('주문 등록 시점의 재고');
